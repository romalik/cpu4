...
void spill_reg16(unsigned char reg)
{
	...
	print("lit off %d\n", get_spill_sp_offset(0)); //m[arrd] = xl
	print("seta %s\n", get16name_low(reg));
	print("puta m[so]\n");
	print("seta %s\n", get16name_high(reg));
	print("lit off %d\n", get_spill_sp_offset(0) + 1); //m[addr+1] = xh
	print("puta m[so]\n");
}
		...
		if ((opsize(p->op)) == 1)
		{
			if ((reg_addr == no_reg) && (use_so == USE_SO_NO))
			{
				...
				if (reg_addr == no_reg)
				{
					...
					fprintf(stderr, "unable to alloc tmp reg for addr, spilling\n");
					spill_reg16(spilled);
					reg_addr = spilled;
	...
	case SUB : 
		...
		else if (opsize(p->op) == 2)
		{
			...
			if ((
							...
							((target_reg == no_reg) ? 1 : 0)) > 1)
			{
				fprint(stderr, "; more than one of reg16 are on stack for alu16, spilling\n");

				//find unused in this op

				if(reg_arg_1 != no_reg) {
					spilled = (1 - reg_arg_1); //just switch to another of two (x,y)
				} else if(reg_arg_2 != no_reg) {
					spilled = (1 - reg_arg_2); //just switch to another of two (x,y)
				} else if(target_reg != no_reg) {
					spilled = (1 - target_reg); //just switch to another of two (x,y)
				} else {
					fprintf(stderr, "all of them are on stack, fail\n");
					assert(0);
				}
			...
			if(spilled != no_reg) {
				if(spilled == reg_arg_1) {
					free_reg16(reg_arg_2);
				} else if(spilled == reg_arg_1) {
					free_reg16(reg_arg_1);
				}
				unspill_reg16(spilled);
			} else {
